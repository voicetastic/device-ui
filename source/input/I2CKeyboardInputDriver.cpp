...
void TDeckKeyboardInputDriver::readKeyboard(uint8_t address, lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    if (Wire.available() > 0 && bytes > 0) {
        ...
        if (keyValue == (char)0xEA) {
            // SYM+space chord (guessed 0xEA). Temporary "play next received
            // voice" hotkey while the chat-screen mini-player widget is being
            // built. The space bar isn't part of the SHIFT+digit 0xE0..0xE9
            // range, so the byte here is a guess — the unmapped-byte log
            // below will print the real value if 0xEA is wrong.
            data->state = LV_INDEV_STATE_RELEASED;
            data->key = 0;
            if (prevTDeckKey != (char)0xEA) {
